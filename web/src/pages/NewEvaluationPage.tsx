...
import { apiFetch, AgentSummary, ApiClientError, BenchmarkSummary, ModelSummary } from "../api/client";
...
export function NewEvaluationPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setBenchmark(String(benchmarkRows[0]?.manifest.benchmark_id ?? ""));
  ...
  const summary = useMemo(() => ({
    ...
    agentParameters,
  }), [agent, agentParameters, benchmark, caseIds, maxCases, maxCost, maxEntryReturn, maxSteps, maxStrLen, maxTokens, model]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const result = await apiFetch<{ run_id: string }>("/api/v1/tasks", { method: "POST", json: {
        ...
        case_ids: caseIds, budget, max_steps: maxSteps,
  ...
  return <section className="page-frame"><PageTitle eyebrow="Queue a run" title="New evaluation" detail="Create a frozen TaskSpec with explicit scope, runtime, limits and budget." />
    ...
    {loading ? <LoadingState label="Loading frozen configuration" /> : <>
      ...
      <form className="form-panel evaluation-form" onSubmit={submit}>
        {step === 0 && <div className="form-grid"><label>Benchmark revision<select aria-label="Benchmark revision" value={benchmark} onChange={(event) => setBenchmark(event.target.value)} required>{benchmarks.map((item) => { const manifest = item.manifest as Record<string, unknown>; return <option value={String(manifest.benchmark_id)} key={String(manifest.benchmark_id)}>{String(manifest.name ?? manifest.benchmark_id)}</option>; })}</select></label><label>Case IDs<input value={cases} onChange={(event) => setCases(event.target.value)} placeholder="smoke-001, smoke-002" required /><span className="field-hint">Comma-separated IDs from the frozen DatasetVersion.</span></label></div>}
        ...
        <div className="form-actions">{step > 0 && <button type="button" className="button button-quiet" onClick={() => setStep((value) => value - 1)}><ChevronLeft size={15} />Back</button>}<button className="button button-primary" disabled={pending || !benchmark || !agent || caseIds.length === 0}>{step < steps.length - 1 ? <>Continue <ChevronRight size={15} /></> : <><Play size={15} />{pending ? "Queueing…" : "Queue evaluation"}</>}</button></div>
      </form>
    </>}
  </section>;
}
